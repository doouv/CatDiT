...
def frac_to_cart_coords(frac_coords, lengths, angles, num_atoms, regularized=True, lattices=None):
    if regularized:
...
def compute_lattice_info(cell, num_atoms, pos):
    """Compute lattice parameters and fractional coordinates"""
    # Handle cell tensor
    cell_np = cell.cpu().numpy() if cell.is_cuda else cell.numpy()
    if cell_np.ndim == 3:
        cell_np = cell_np.squeeze(0)

    # Lattice calculations
    a, b, c, alpha, beta, gamma = lattice_matrix_to_params(cell_np)
    lengths = torch.tensor([a, b, c], dtype=torch.float32)
    angles = torch.tensor([alpha, beta, gamma], dtype=torch.float32)
    lattice = lattice_params_to_matrix(a, b, c, alpha, beta, gamma)
...
def create_base_data_object(atom_types, pos, frac_coords, tags, cell, lattice_info, num_atoms):
    """Create base PyG Data with common fields (A, X, F, L, T)"""
    return Data(
        # A, X, F, T
        ...
        tags=tags.long(),
        # L
        cell=cell.unsqueeze(0) if cell.ndim == 2 else cell,
        lattices=torch.from_numpy(lattice_info['lattice']).unsqueeze(0).float(),
        lattices_scaled=torch.cat([lattice_info['lengths_scaled'], lattice_info['angles_radians']]).unsqueeze(0),
        lengths=lattice_info['lengths'].unsqueeze(0),
        lengths_scaled=lattice_info['lengths_scaled'].unsqueeze(0),
        angles=lattice_info['angles'].unsqueeze(0),
        angles_radians=lattice_info['angles_radians'].unsqueeze(0),
        # for sampling
