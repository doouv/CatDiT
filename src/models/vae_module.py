...
from src.eval.catalyst_reconstruction import CatalystReconstructionEvaluator
...
class DiagonalGaussianDistribution:
    ...
    def kl(self, other=None):
        if self.deterministic:
            ...
            if other is None: # other(prior distribution) = N(0,1)
...
```
    ...
    def __init__(
        ...
        self.loss_weights_tags = torch.nn.Parameter(
            torch.tensor([*self.loss_weights["loss_tags"].values()]),
            requires_grad=False,
        )
        ...
        self.val_reconstruction_evaluators = {
            "oc22": CatalystReconstructionEvaluator(),
            "oc20": CatalystReconstructionEvaluator()
        }
    ...
    def reconstruction_criterion(
        ...
        # Tags loss
        loss_tags = F.cross_entropy(out["tags"], batch.tags, reduction="none")
    ...
    def criterion(
        ...
        weights_tags = self.loss_weights_tags[batch.dataset_idx[batch.batch]]
        ...
        loss = (
            ...
            + (weights_tags * loss_reconst["loss_tags"]).mean()
        ...
        return {
            ...
            "loss_tags": weights_tags * loss_reconst["loss_tags"],
            ...
            "unscaled/loss_tags": loss_reconst["loss_tags"],
    ...
    def on_evaluation_epoch_start(self, stage: Literal["val", "test"]) -> None:
        ...
        for dataset in reconstruction_evaluators.keys():
            ...
            pred_count = len(reconstruction_evaluators[dataset].pred_arrays_list)
            gt_count = len(reconstruction_evaluators[dataset].gt_arrays_list)
    ...
    def evaluation_step(self, batch, batch_idx, dataloader_idx, stage):
        ...
        reconstruction_evaluator = getattr(self, f"{stage}_reconstruction_evaluators")[dataset_name]
        ...
        # update and log metrics (LOG HERE like original code!)
        for k, v in loss_dict.items():
            metrics[k](v)
            self.log(
                f"{stage}_{dataset_name}/{k}",
                metrics[k],
                on_step=False,
                on_epoch=True,
                prog_bar=False,
        ...
        for idx_in_batch, num_atom in enumerate(batch.num_atoms.tolist()):
            ...
            _atom_types[_atom_types == 0] = 1
            _tags = (
                out["tags"].narrow(0, start_idx, num_atom).argmax(dim=1)
            )
        ...
        # Save groundtruths for metrics and visualisation
            ...
            reconstruction_evaluator.append_gt_array(
                {
                    ...
                    "tags": _data["tags"].detach().cpu().numpy(),
    ...
    def on_evaluation_epoch_end(self, stage: Literal["val", "test"]) -> None:
        ...
        for dataset in metrics.keys():
            if len(reconstruction_evaluators[dataset].pred_arrays_list) == 0:
                continue
            ...
            try:
                ...
                print(f"[ERROR] Failed to compute metrics for {dataset}: {e}")
                import traceback
                traceback.print_exc()

            if self.hparams.visualization.visualize and type(self.logger) == WandbLogger:
                try:
                    print(f"[INFO] Rank {self.global_rank} - Creating WandB table for {dataset}...")
                    pred_table = reconstruction_evaluators[dataset].get_wandb_table(
                        current_epoch=self.current_epoch,
                        save_dir=self.hparams.visualization.save_dir
                        + f"/{dataset}_{stage}_{self.global_rank}",
                    )
                    print(f"[INFO] Rank {self.global_rank} - Table created successfully, logging to WandB...")
                    self.logger.experiment.log(
                        {f"{dataset}_{stage}_pred_table_device{self.global_rank}": pred_table}
                    )
                    print(f"[INFO] Rank {self.global_rank} - Table logged successfully!")
                except Exception as e:
                    print(f"[ERROR] Rank {self.global_rank} - Failed to create/log WandB table for {dataset}: {e}")
                    import traceback
                    traceback.print_exc()
