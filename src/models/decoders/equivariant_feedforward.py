...
class FeedForwardDecoder(nn.Module):
    ...
    def __init__(
        ...
        # Tag prediction head
        self.tags_head = FeedForwardNetwork(
            self.sphere_channels,
            self.ffn_hidden_channels,
            3, # ads, surface,fixed
            self.lmax_list,
    ...
    def forward(self, encoded_batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # if self.enable_hook:
        #     self.hooked_x_embedding = x.embedding
        ...
        # Tags prediction head
        tags_out = self.tags_head(x).embedding.narrow(1, 0, 1).squeeze(1)
