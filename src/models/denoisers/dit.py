...
class ScalarEmbedder(TimestepEmbedder):
    """Embeds continuous scalars into vector representation using TimestepEmbedder's sinusoidal encoding.
    Also handles scalar dropout for classifier-free guidance."""
    ...
    def forward(self, scalars, train, force_drop_ids=None):
        embeddings = super().forward(scalars)

        # Drop logic
        if force_drop_ids is not None:
            drop_ids = (force_drop_ids == 1)
        elif train and self.dropout_prob > 0:
            drop_ids = torch.rand(scalars.shape[0], device=scalars.device) < self.dropout_prob
        else:
            # Add small contribution from null_embedding to ensure it's always used
            # This prevents DDP unused parameter errors
            embeddings = embeddings + 0.0 * self.null_embedding[0]
            return embeddings  # No dropout needed

        # Apply null embedding where needed
        if drop_ids.any():
            null_expanded = self.null_embedding.unsqueeze(0).expand_as(embeddings)
            embeddings = torch.where(drop_ids.unsqueeze(-1), null_expanded, embeddings)
        else:
            embeddings = embeddings + 0.0 * self.null_embedding[0]
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        scalar_dropout_prob=0.1,
        num_ads= 82,
        num_cat_classes= 5,  # clf-free guidance input
    ):
        super().__init__()
        self.d_x = d_x
        ...
        self.be_embedder = ScalarEmbedder(d_model, scalar_dropout_prob)
        self.ads_embedder = LabelEmbedder(num_ads, d_model, class_dropout_prob)
        self.cc_embedder = LabelEmbedder(num_cat_classes, d_model, class_dropout_prob)
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.cc_embedder.embedding_table.weight, std=0.02)
        nn.init.normal_(self.ads_embedder.embedding_table.weight, std=0.02)

        # Initialize binding energy MLP:
        nn.init.normal_(self.be_embedder.mlp[0].weight, std=0.02)  # Linear layer
        nn.init.normal_(self.be_embedder.mlp[2].weight, std=0.02)
    ...
    def forward(self, x, t, binding_energy, cat_class, ads_id, mask, x_sc=None, force_drop_ids=None, cond_scale=1.0):
        """Forward pass of DiT.
        ...
        Args:
            ...
            force_drop_ids (torch.Tensor): Force specific samples to use null embeddings for CFG
            cond_scale (float): Scale for condition embeddings (0→1 warmup during fine-tuning)
        """
        ...
        t_emb = self.t_embedder(t.squeeze(1))  # (B, d)
        be = self.be_embedder(binding_energy, self.training, force_drop_ids)  # (B, d)
        cc = self.cc_embedder(cat_class, self.training, force_drop_ids)  # (B, d)
        ads = self.ads_embedder(ads_id, self.training, force_drop_ids)  # (B, d)
        # Warmup: cond_scale 0→1 allows gradual transition from unconditional to conditional
        c = t_emb + cond_scale * (be + cc + ads)  # (B, d)
    ...
    def forward_with_cfg(self, x, t, binding_energy, cat_class, ads_id, mask, cfg_scale, x_sc=None, cond_scale=1.0):
        ...
        force_drop_ids = torch.cat([
            ...
            torch.ones(batch_size, device=x.device, dtype=torch.long)], dim=0)

        half_x = x[: len(x) // 2]
        combined_x = torch.cat([half_x, half_x], dim=0)
        model_out = self.forward(combined_x, t, binding_energy, cat_class, ads_id, mask, x_sc, force_drop_ids=force_drop_ids, cond_scale=cond_scale)
