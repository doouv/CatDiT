...
class FlowMatchingInterpolant:
    ...
    def _sample_t(self, batch_size):
        ...
        # t = torch.distributions.Beta(0.5, 1.0).sample((batch_size,)).to(self.device)
        # return t * (1 - 2 * self.min_t) + self.min_t
    ...
    def sample_with_classifier_free_guidance(
        ...
        Args:
            ...
            model (nn.Module): Denoiser model to use.
            ads_id (torch.Tensor): Adsorbate ID corresponding to symbol, used for classifier-free guidance. (B, 1)
            cat_class (torch.Tensor): Catalyst class, used for classifier-free guidance. (B, 1)
            binding_energy (torch.Tensor): Binding energy, used for classifier-free guidance (B, 1).
        ...
        ads_id_null = torch.zeros_like(ads_id)
        ads_id = torch.cat([ads_id, ads_id_null], dim=0)  # (2B, 1)
        cat_class_null = torch.zeros_like(cat_class)
        cat_class = torch.cat([cat_class, cat_class_null], dim=0)  # (2B, 1)
        binding_energy_null = torch.zeros_like(binding_energy) # will be replaced by learnable null during DiT forward process
        binding_energy = torch.cat([binding_energy, binding_energy_null], dim=0)
        ...
        for t_2 in ts[1:]:
            ...
            with torch.no_grad():
                pred_x_1 = model.forward_with_cfg(
                    x, t, binding_energy, cat_class, ads_id, token_mask, cfg_scale, x_sc
                )
