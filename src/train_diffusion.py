...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # Load weights only (for fine-tuning without restoring optimizer/scheduler)
    if cfg.get("load_weights_only") and cfg.get("ckpt_path"):
        log.info(f"Loading weights only from {cfg.get('ckpt_path')}")
        ckpt = torch.load(cfg.get("ckpt_path"), map_location="cpu")
    ...
    if cfg.get("train"):
        ...
        # If load_weights_only, don't pass ckpt_path (already loaded above)
        fit_ckpt_path = None if cfg.get("load_weights_only") else cfg.get("ckpt_path")
